...
const { MICROSOFT_GRAPH_CLIENT_ID, MICROSOFT_GRAPH_CLIENT_SECRET, MICROSOFT_GRAPH_TENANT_ID, } = process.env;
...
class BookingsServer {
            ...
            try {
                switch (request.params.name) {
                    ...
                    case 'get_business_appointments': {
                        const args = request.params.arguments;
                        return await this.getBusinessAppointments(args.businessId, args.startDate, args.endDate);
                    }
                    default:
    ...
    async getBusinessAppointments(businessId, startDate, endDate) {
        let api = this.graphClient
            .api(`/solutions/bookingBusinesses/${businessId}/appointments`);
        if (startDate || endDate) {
            const filter = [];
            if (startDate)
                filter.push(`start ge ${startDate}`);
            if (endDate)
                filter.push(`end le ${endDate}`);
